...
## ロボットモード
- [x] 巨大ロボット（歩行・ダッシュ・バーニア飛行・屋上着地）、ビーム、建物の崩壊と瓦礫
- [x] 6 棟で怪獣出現、体力ゲージ（画面上部＋VR は頭上）、撃破で街とロボットを元に戻す
- [x] explore にロボットの 1 ラウンドを追加（3 項目）
- [ ] VR 実機でロボットの操作と酔いの確認（プラズマ弾で吹き飛ばされる動き、前傾）
- [x] 効果音（Web Audio で合成。ビーム・着弾・崩落・足音・着地・バーニア・怪獣・チャイム、車のエンジンと風）
- [ ] 効果音の音量バランスを実際に聞いて調整（合成と配線はテストで確認済み、耳では未確認）
- [ ] ロボットの体力と負け（今は吹き飛ばされるだけで負けは無い）
