...
| 空中でライフルが的より下を向き、ゆっくり上がる | 腕の狙いが、ロボットの前傾に上書きされる前の `body.rotation.x`（上昇下降の傾き）を読んでいた。前傾は照準の前にセットする |
| 近接コンボが毎回構えに戻り、繋がって見えない | 各技の終わり＝次の技の始まりになるよう振付を並べ、次の技は follow-through から直接始める（構えに戻るのはコンボが途切れた時だけ） |
## 見つけた罠はここに追記していく
