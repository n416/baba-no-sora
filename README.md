...
| サーベルの連続技（等速） | 両手持ち（1/3 スロー） |
|---|---|
| ![連続技](docs/media/saber-combo.gif) | ![両手持ち](docs/media/saber-twohand.gif) |
| **ブースト中の集中線**（ロボット中心） | **自由落下と着地前の減速噴射、上昇中からの急降下** |
| ![集中線](docs/media/boost-lines.gif) | ![落下と急降下](docs/media/fall-dive.gif) |

- **ビーム**：左クリック（押しっぱなしで連射）。照準は画面中央。低いビルは 1 発、高いビルは 2〜3 発で崩れる。
  ビームは光弾として飛んでいき（秒速 180 m）、少し遅れて着弾する。発射の閃光、光の尾、着弾の衝撃波リングと火花。
  撃つと右腕（ビーム砲）が上がってターゲットへ向き、体もだんだん旋回する。腕が届かない間は腕の向いている先（近い位置）に当たり、
  向き切ると狙った所に当たる。**真後ろ（正面から 100° より後ろ）には撃てない**（先に旋回する）
- **バーニア**：Space・E で上昇。離すと重さのまま落ちて、着地の手前でバーニアが自動で噴いて減速する。Ctrl・Q で急降下（慣性があるので上昇中に押してもいきなりは落ちない。減速せずに叩きつけて、下のビルを踏み潰す）。屋上にも降りられる
...
- **接近モード**：怪獣に 50 m まで近づくと、背中からビームサーベルを抜いて接近モードに切り替わる（離れると背中に戻して射撃モード）。
  左クリックで斬る。**続けて押すと技が変わる**：袈裟斬り → 横薙ぎ → 斬り上げ → 唐竹割り（とどめ、大きく吹き飛ばす）。
  各技は前の技が終わった位置から始まる（左下へ斬り下ろす → 左から右へ薙ぐ → 右下から斬り上げる → 頭上から振り下ろす）ので、
  連続技の間は構えに戻らず、踏み込んだ前足に体を運びながら前進し続ける。横薙ぎ・唐竹割り・構えは空いた左手を柄に添えた両手持ち。
  手を止めると振り抜いた姿勢のまま一瞬止まり（余韻）、ゆっくり構えに戻る。戻り始めてから押すと最初の技から。
  当たった瞬間に一瞬止まるヒットストップと画面の揺れ。刃の軌跡が光の弧として残る。怪獣の体にはめり込まない
- **怪獣がのけぞる**：当たるたびに上体をのけぞらせて口を開け、後ずさりしてよろめく。強い技ほど大きく、プラズマのチャージも止まる
...
| W S / A D | 前後 / 旋回 |
| マウス | 照準（カメラは右肩越し） |
| 左クリック | ビーム（射撃モード）／ サーベルで斬る・連打で連続技（接近モード） |
| Shift | ダッシュ（空中では加速）。続けると画面にロボット中心の集中線 |
| Space・E / Ctrl・Q | バーニア上昇 / 急降下 |
| F | 乗り降り（地上で止まっているとき） |

**効果音**：ビーム、着弾、ビルの崩落（地響きとコンクリートの割れる音）、ロボットの足音と着地、バーニアの噴射音、
怪獣の咆哮・足音・プラズマのチャージと発射、勝利のチャイム。翼のある車はエンジンとプロペラ、飛行中の風切り音。
音はすべてその場で合成していて音声ファイルはありません。音源の位置で左右と遠近が変わります。M で ON/OFF。
